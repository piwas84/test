...
(function () {
  ...
  function install() {
    ...
    // Зберігаємо поточні версії (навіть якщо їх уже хтось перехопив)
    originalActivityPush = Lampa.Activity.push;
    originalParserGet = Lampa.Parser.get;
    ...
    Lampa.Activity.push = function () {
      ...
      // Якщо вже йде наш тест — пропускаємо
      if (opening && canMeasure(object)) {
        return;
      }

      // Якщо фільтр вимкнений або не підходить — передаємо далі по ланцюжку
      if (!enabled() || !canMeasure(object) || object._speed_filter_checked) {
        return originalActivityPush.apply(context, args);
      }
      ...
      function resume(speed) {
        ...
        setTimeout(function () {
          ...
          // Викликаємо оригінальний (або попередній) Activity.push
          originalActivityPush.apply(context, args);
        }, 50);
      }
    ...
    Lampa.Parser.get = function () {
      ...
      // Якщо умови не виконані — просто передаємо далі
      if (
        !enabled() ||
        !isFinite(limit) ||
        limit <= 0 ||
        ...
        return originalParserGet.apply(context, args);
      }
      ...
      // Викликаємо оригінальний Parser.get з нашим обгорнутим success
      return originalParserGet.apply(context, args);
    };
  }
